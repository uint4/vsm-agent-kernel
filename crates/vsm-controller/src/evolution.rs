...
use vsm_core::{
    score_trace, GeneSuggestion, GeneSuggestionSource, NodeId, NodeLifecycleStatus,
    OrganizationalGenome, OrganizationalGenomePatch, PromptComponent, PromptOrigin, PromptSection,
    SuggestionId, TaskTrace, TrialMode, ViableNode,
};
use vsm_ledger::{
...
mod tests {
    use super::*;
    use vsm_core::{GenomeId, LeafOperationSpec, TaskId};
    ...
    fn token_heavy_trace(genome_id: GenomeId, node_id: NodeId) -> TaskTrace {
    ...
    #[test]
    fn generation_adds_structural_prompt_guardrail_from_token_pressure() {
        let (genome, root_id, coder_id) = champion();
        let traces = (0..5)
            .map(|_| token_heavy_trace(genome.id.clone(), coder_id.clone()))
            .collect::<Vec<_>>();
        ...
        assert!(plan
            .suggestions
            .iter()
            .any(|suggestion| suggestion_operator(suggestion) == "add_prompt_parsimony_guardrail"));
    ...
    #[test]
    fn generation_does_not_create_semantic_review_or_test_roles_from_pressure() {
        let (genome, root_id, coder_id) = champion();
        let traces = (0..5)
            .map(|_| {
                let mut trace =
                    TaskTrace::started(TaskId::new(), genome.id.clone(), coder_id.clone());
        ...
        assert!(plan
            ...
            .all(|suggestion| {
                !matches!(
                    suggestion_operator(&suggestion),
                    "add_child_reviewer" | "add_child_tester"
                )
            }));
    }
}
