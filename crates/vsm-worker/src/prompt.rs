...
fn build_instructions(node: &ViableNode) -> String {
    ...
    out.push_str("You are an operational leaf in a recursive Viable System Model coding organization.\n");
    ...
    out.push_str(&format!("Denied paths: {:?}\n", node.permissions.denied_paths));
    ...
    append_section(&mut out, "Codebase conventions", &node.prompt.codebase_conventions);
    append_section(&mut out, "Negative constraints", &node.prompt.negative_constraints);
